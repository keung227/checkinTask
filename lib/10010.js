...
var UNICOM = function() {
      ...
      for(var i = 0; i < count; i++) {
        var response = await request
          ...
          .send({
            ...
            deviceModel: 'iPhone',
            keyVersion: 1,
            deviceCode: '2006B428-F6F6-488B-903A-D2A60267F2EA'
          })
  ...
  self.querySigninActivity = async (mobile, token, userAgent, timeout, count) => {
    try {
      for(var i = 0; i < count; i++) {
        var response = await request
          .get(Buffer.from('aHR0cHM6Ly9hY3QuMTAwMTAuY29tL1NpZ25pbkFwcC9zaWduaW4vcXVlcnlTaWduaW5BY3Rpdml0eS5odG0=', 'base64').toString('utf8'))
          .query({
            token: token
          })
          .set('User-Agent', userAgent)
  ...
  self.daySign = async (mobile, token, daySignCookies, userAgent, timeout, count) => {
    try {
      for(var i = 0; i < count; i++) {
        var response = await request
          .post(Buffer.from('aHR0cHM6Ly9hY3QuMTAwMTAuY29tL1NpZ25pbkFwcC9zaWduaW4vZGF5U2lnbi5kbw==', 'base64').toString('utf8'))
          .set('User-Agent', userAgent)
